...
"""
'''
* Last modified at 23/11/2025 by Carolina Tatiani
'''
...
import shutil
import mdtraj as md
from force_scheme import FS, GFS, SFS
import sys

@njit(parallel=True, fastmath=False)         ####dissimilarity default, generalizar para diferentes sigma_0 e epsilon
...
def main():
    ...
    Args:
        ...
        -m : Model to be used: FS (Force-Scheme), GFS (Gradient Force-Scheme), SFS (Scalable Force-Scheme) (default: SFS)
        -n (int): Number of dimensions for the projection (default: 2)
        -z (filename): Fix the z coordinate to the values in the provided file. If not provided, the giration radius is used to set the z coordinate (trajectory file required). 
    Returns:
  ...
  """
    ...
    parser.add_argument("-m", dest="model", action="store", type=str, default="SFS",
                  help="Model to be used: FS (Force-Scheme), GFS (Gradient Force-Scheme), SFS (Scalable Force-Scheme) or (default: SFS)")
    parser.add_argument("-n", dest="n_components", action="store", type=int, default=2,
                  help="Number of dimensions for the projection (default: 2)")
    parser.add_argument("-z", dest="z_fixed_file", action="store", type=str,
                  help="Fix the z coordinate to the values in the provided file. If not provided, the giration radius is used to set the z coordinate (trajectory file required).")
    ...
    z_fixed_file=args.z_fixed_file
...
#### Check if only one of the possible files are parsed to avoid ambiguities between them.    
  ...
  ##  Check if the z coordinate will be fixed
    if z_fixed_file!=None:
        try:
            z_fixed = np.loadtxt(z_fixed_file)
            if len(z_fixed.shape)!=1 or z_fixed.shape[0]!=size:
                raise RuntimeError("Dimension error: the z coordinate file should contain only one column with {} values \n".format(size))
            print('Z coordinate fixed to the values in the file {} \n'. format(z_fixed_file))        
        except:
            raise RuntimeError('''Failed to load the z coordinate file. 
                                  Make sure it is a text file with one column and {} values \n'''.format(size))
    else:
        n_components=2  # Projection in 2D if file is not provided
        print('Projection in 2D since no trajectory or fixed coordinates are provided \n')
    ...
    print('ELViM projection will be performed using the {} model \n'. format(model))
    if model=="FS":
        method = FS(max_it=max_it,
                    learning_rate0=lr0,
                    decay=decay,
                    tolerance=tol,
                    n_components=n_components)
    elif model=="GFS":
        method = GFS(max_it=max_it,
                     learning_rate0=lr0,
                     decay=decay,
                     tolerance=tol,
                     n_components=n_components)
    elif model=="SFS":
        method = SFS(max_it=max_it,
                     learning_rate0=lr0,
                     decay=decay,
                     tolerance=tol,
                     n_components=n_components)
    else:
        raise RuntimeError("Model not recognized. Choose between: FS, GFS or SFS \n")
    
    nr_iterations, error, kstress = method.fit_transform(dmat, projection, verbose=verbose)
    ...
    with open(outp, 'w') as f:
       ...
       f.write("# Used the {} model to perform {} iterations with lr0 = {:5.3f}, lrmin = {:5.3f}, and decay = {:5.3f} \n".format(model, nr_iterations, lr0, lrmin, decay))
